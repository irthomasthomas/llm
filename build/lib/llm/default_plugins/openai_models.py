...
class SharedOptions(llm.Options):
    ...
    @field_validator("top_logprobs", "logprobs")
    def validate_logprob_settings(cls, v, info):
        # Get the model data object being validated
        data = info.data
        ...
        if field == "top_logprobs":
            if v is not None:
                ...
                # Ensure logprobs is also enabled
    ...
    def build_kwargs(self, prompt, stream):
        ...
        if "logprobs" in kwargs:
            ...
            if isinstance(self, Chat) and not isinstance(self, Completion):
                # For chat models, convert logprobs to integer for top_logprobs
                if isinstance(logprobs, bool):
                    kwargs.pop("logprobs")
                    if logprobs:
                        kwargs["top_logprobs"] = top_logprobs or 1
                elif isinstance(logprobs, int):
                    kwargs.pop("logprobs")
                    kwargs["top_logprobs"] = logprobs
                # Always set logprobs=True when top_logprobs is set
                if "top_logprobs" in kwargs:
                    kwargs["logprobs"] = True
            else:
                # For completion models, keep original logprobs handling
        ...
        for choice in item.choices:
            if hasattr(choice, "logprobs") and choice.logprobs:
                if hasattr(choice.logprobs, "content"):
                    # Handle chat completion logprobs
